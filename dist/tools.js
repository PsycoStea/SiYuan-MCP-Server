 ...
 */
import { CATEGORIES } from "./notebook-manager.js";
...
export function getToolDefinitions() {
    return [
        ...
        {
            name: "siyuan_search",
            description: "Full-text search across ALL SiYuan notebooks (including read-only ones). " +
            ...
            inputSchema: {
                ...
                properties: {
                    ...
                    notebooks: {
                        type: "array",
                        items: { type: "string" },
                        description: "Optional: restrict to specific notebook names, e.g. [\"AI Convos\", \"General\", \"AI Memory\"]",
                    },
                },
                required: ["query"],
            },
        ...
        // ── READ: Get a specific memory entry ──────────────────────────────────
        {
            ...
            description: "Retrieve a specific entry from the AI Memory notebook by category and key.",
            inputSchema: {
                type: "object",
                properties: {
                    category: {
                    ...
                    key: {
                        type: "string",
                        description: "Entry key/slug, e.g. \"my-app\", \"dark-mode\", \"use-pnpm\"",
                    },
                },
        ...
        // ── READ: List all memory entries in a category ─────────────────────────
        {
            ...
            description: "List all memory entries in a category, returning title, key, tags, and updated_at. " +
        ...
        // ── READ: Bulk load context ─────────────────────────────────────────────
        {
            ...
            description: "Load a summary of ALL AI Memory entries across all categories. " +
        ...
        // ── WRITE: Upsert a memory entry ────────────────────────────────────────
        {
            ...
            description: "Save or update an entry in the AI Memory notebook. " +
            ...
            inputSchema: {
                ...
                properties: {
                    ...
                    key: {
                        type: "string",
                        description: "Unique slug for this entry, e.g. \"my-app\", \"prefers-tabs\". " +
                    ...
                    data: {
                        type: "object",
                        description: "The structured data to store. Use descriptive keys. " +
        ...
        // ── WRITE: Delete a memory entry ────────────────────────────────────────
        {
            ...
            description: "Delete an entry from the AI Memory notebook. " +
        ...
        // ── CONFIRM-REQUIRED: Request write access to user notebooks ───────────
        {
            ...
            description: "Request permission to write to a user notebook ('AI Convos' or 'General'). " +
            ...
            inputSchema: {
                ...
                properties: {
                    ...
                    operation_description: {
                        type: "string",
                        description: "Plain-English description of exactly what you want to write/change",
                    },
                    document_path: {
                        type: "string",
        ...
        // ── CONFIRM-REQUIRED: Confirm a pending write ───────────────────────────
        {
            ...
            description: "Confirm and execute a previously requested write operation using its token. " +
        ...
        {
            name: "siyuan_notify",
            description: "Push a notification message to the SiYuan desktop UI. " +
...
// ─── Tool handler ─────────────────────────────────────────────────────────────
export async function handleTool(name, args, client, manager) {
    cleanExpired();
    switch (name) {
        // ── siyuan_search ───────────────────────────────────────────────────────
        case "siyuan_search": {
            const query = args.query;
            const notebookNames = args.notebooks;
            let notebookIds;
            ...
            return {
                ...
                results: results.map((r) => ({
                    id: r.id,
                    notebook: nbNameMap[r.box] ?? r.box,
                    notebook_id: r.box,
                    readonly: client.isReadonly(r.box),
                    hpath: r.hpath,
                    type: r.type,
        ...
        case "siyuan_save_memory": {
            const { category, key, title, data, tags } = args;
            const result = await manager.upsertEntry(category, key, title, data, tags ?? []);
            // Push a quiet notification to SiYuan UI
            const action = result.created ? "Created" : "Updated";
            await client.pushNotification(`[AI Memory] ${action}: ${category}/${key} — "${title}"`);
            return {
                success: true,
        ...
        case "siyuan_request_write": {
            const { notebook_name, operation_description, document_path, content, } = args;
            // Resolve notebook ID
            const allNbs = await client.listNotebooks();
            ...
            pendingOps.set(token, {
                ...
                execute: async () => {
                    ...
                    if (existingIds && existingIds.length > 0) {
                        ...
                        "CONFIRMED_BYPASS_" + nb.id, existingIds[0], content);
                    }
                    else {
                        await client.createDoc("CONFIRMED_BYPASS_" + nb.id, document_path, content);
                    }
